"""Train the Seq2Seq encoder-decoder model for CO2 forecasting.

The Seq2Seq model uses an autoregressive decoder with Bahdanau attention,
designed to improve long-horizon forecasting (e.g., 24h) where the direct
forecasting approach (single Linear layer) fails.

Usage:
    python scripts/train_seq2seq.py
    python scripts/train_seq2seq.py --horizon 1
    python scripts/train_seq2seq.py --horizon 24
    python scripts/train_seq2seq.py --horizon 1 24 --experiment configs/experiments/preproc_E_occupancy_1h.yaml
"""
...
def train_single_horizon(config: dict) -> None:
    """Train Seq2Seq for a single forecast horizon.
    ...
    # Model
    model = Seq2SeqForecaster(config)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Seq2Seq encoder-decoder model")
    ...
    config_files = [
        ...
        str(PROJECT_ROOT / "configs" / "seq2seq.yaml"),
    ]
    if args.experiment:

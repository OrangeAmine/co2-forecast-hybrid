"""Train the CatBoost model for CO2 forecasting.

Usage:
    python scripts/train_catboost.py
    python scripts/train_catboost.py --horizon 1
    python scripts/train_catboost.py --horizon 24
"""
...
def train_single_horizon(config: dict) -> None:
    """Train CatBoost for a single forecast horizon.
    ...
    base_name = f"CatBoost_h{horizon}"
    ...
    # Extract numpy arrays from datasets (asserts already above)
    ...
    t0 = time.time()
    print(f"  Fitting CatBoost with {model.iterations} iterations, "
          f"depth={model.depth}...")
...
def main() -> None:
    ...
    parser.add_argument("--epochs", type=int, default=None,
                        help="Override max epochs (controls iterations)")
    ...
    for horizon in args.horizon:
        ...
        if args.epochs:
            config["model"]["iterations"] = args.epochs
        if args.lookback:
